            ...
            jwt.verify(req.token, 'Y2Ae7kXZ', (err, authData) => {
                if (err) {
                    ...
                    conn.connect().then(function() {
                        ...
                        request.input('ContractorID', sql.Int, req.body.contractorId);
                        request.input('rate', sql.Int, req.body.rate);
                        request.input('comment', sql.VarChar(250), req.body.comment);
...
exports.getContractorRating = function(req) {
    return new Promise((resolve, reject) => {
        var data = {};
        data.msg = { Code: 200, Message: 'Exito!', Tipo: 'n/a' };
        var conn = config.findConfig();
            ...
            jwt.verify(req.token, 'Y2Ae7kXZ', (err, authData) => {
                if (err) {
                    ...
                    conn.connect().then(function() {
                        ...
                        request.execute("dbo.sp_getContractorReview").then(function(recordsets) {
                            let rows = recordsets.recordset;
                            var mainKey = rows[0];
                            var selectedKey;
...
exports.getCustomerReview = function(req) {
    return new Promise((resolve, reject) => {
        var data = {};
        data.msg = { Code: 200, Message: 'Exito!', Tipo: 'n/a' };
        var conn = config.findConfig();
            ...
            jwt.verify(req.token, 'Y2Ae7kXZ', (err, authData) => {
                if (err) {
                    ...
                    conn.connect().then(function() {
                        ...
                        request.execute("dbo.sp_getCustomerReview").then(function(recordsets) {
                            let rows = recordsets.recordset;
                            var mainKey = rows[0];
                            var selectedKey;
                            for (var key in mainKey) {
